...
public class DecreaseVotePointsCommand implements Command {
	private int run(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
		...
		UUID userId;
		if (context.getNodes().size() > 3) { // Check if username argument is present
		...
		user.decreaseVotePoints(points);

		if (player != null) {
			new Message("You have been deducted " + points + " vote points.").send(player, false);
		} else {
			source.sendFeedback(Text.literal("You have deducted " + points + " vote points from " + username), false);
		}
		return 1;
	}
